...
# isolated per environment — applying here can never overwrite qa/staging/prod.
#
# Before running:
#
...
#   3. The workspace "marketo-aurora-mysql-dev" is created automatically on
#      first apply.
...
terraform {
  cloud {
    organization = "pocwithmehul"

    workspaces {
      name = "marketo-aurora-mysql-dev"
    }
  }
